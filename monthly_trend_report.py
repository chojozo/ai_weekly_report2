...
def get_recent_articles():
    ...
    response = supabase.table("articles").select("title, link").gte("created_at", one_month_ago.isoformat() + "Z").execute()
...
다음에 제시되는 AI 관련 뉴스 제목과 링크 목록을 기반으로, 지난 1개월 동안의 주요 AI 뉴스를 요약 작성해 주세요.
...
각 트렌드 아래에는 불릿을 붙여 3~5개 주요 뉴스를 설명해 주세요.
설명 문장 내의 핵심 키워드를 클릭하면 관련 기사로 연결되도록 해주세요.

--- (마크다운 가로줄)

## 주요 기업 동향
google, Microsoft, OpenAI, meta, alphabet 등 주요 기업의 최근 동향을 포함해 주세요.
문장에는 불릿을 붙여 작성해 주고, 문장 내의 핵심 키워드를 클릭하면 관련 기사로 연결되도록 해주세요.

--- (마크다운 가로줄)

## 기술 트렌드
새롭게 발표된 기술이나 제품, 모델, 연구 결과 등에 대한 기사를 설명해 주세요.
문장에는 불릿을 붙여 작성해 주고, 문장 내의 핵심 키워드를 클릭하면 관련 기사로 연결되도록 해주세요.
...
if __name__ == "__main__":
    ...
    if not articles:
        ...
        if report_content:
            report_content += (
                "\n\n---\n\n"
                "본 보고서는 국내외 주요 AI 전문 언론사의 최근 1개월 기사 내용을 기반으로 **ChatGPT**가 종합·작성한 자료입니다.\n"
                "*(국내: AI TIMES, Mirakle AI, 로봇신문 / 해외: MIT Technology Review, The Verge, VentureBeat, Techcrunch)*"
            )
            print("보고서 생성 완료. Notion 페이지를 생성합니다...")
            page_title = f"월간 AI 트렌드 분석 보고서 ({datetime.now().strftime('%Y-%m')})"  # NEW
